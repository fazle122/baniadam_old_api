...
class TrackableEmployeeFilterDialog extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return _TrackableEmployeeFilterDialogState();
  }
}

class _TrackableEmployeeFilterDialogState extends BaseState<TrackableEmployeeFilterDialog> {
...
//    Map<String, dynamic> data = await ApiService.getBranchTypes();
    List<dynamic> data = await ApiService.getBranchTypes1();
    if (data != null) {
      setState(() {
...
//    Map<String, dynamic> data = await ApiService.getDepartments();
    List<dynamic> data = await ApiService.getDepartments1();
    if (data != null) {
      setState(() {
// //       data['attributes'].forEach((k,v) => _reasons.addAll(k,v))
        for (int i = 0; i < data.length; i++) {
...
//    Map<String, dynamic> data = await ApiService.getCategories();
    List<dynamic> data = await ApiService.getCategories1();
    if (data != null) {
      setState(() {
//        data['attributes'].forEach((k,v) => _reasons.addAll(k,v))
        for (int i = 0; i < data.length; i++) {
