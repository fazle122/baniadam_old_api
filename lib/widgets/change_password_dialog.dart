...
//      Toast.show("Confirm password didn\'t matched", context,
//          duration: Toast.LENGTH_LONG, gravity: Toast.BOTTOM);
//    }else {
    if (_formKey.currentState.validate()) {
      final String loginResponse = await ApiService.changePassword(
          _oldPassController.text, _newPassController.text);

      if (loginResponse != null && loginResponse != '') {
        print(loginResponse);
        String message = loginResponse.substring(8, 24);
                            ...
                            if (_formKey.currentState.validate()) {
                              if (_newPassController.text.length < 6) {
                                ...
                                if (loginResponse != null &&
                                    loginResponse != '') {
                                  print(loginResponse);
                                  String message = loginResponse.substring(
                                      8, loginResponse.length - 2);
                                  Toast.show(message, context,
                                      duration: Toast.LENGTH_LONG,
                                      gravity: Toast.CENTER);
                                }
                                if (loginResponse == '') {
                                  Toast.show(
                                      "Failed to Change password", context,
                                      duration: Toast.LENGTH_LONG,
                                      gravity: Toast.CENTER);
                                }
                              }
